...
const Profile = ({summoner, aggregateRankedStats, rankedLeagues}) => {
  if (
    ...
    rankedLeagues[summoner.id]
  ) {
    return (
      <Panel
        header={<h3>{summoner.name || 'loading...'}</h3>}
        bsStyle="primary"
      >
        <div>
          <div className="text-center ">
            <img
              src={
                `https://ddragon.leagueoflegends.com/cdn/6.12.1/img/profileicon/${summoner.profileIconId || '666'}.png`
              }
            />
          </div>
          <ProfileRankedStats
