  ...
  if (rankedStats && summoner.summonerLevel == 30) {
    return (
      ...
      >
        ...
        >
          <TableHeaderColumn
            dataFormat={championIconFormatter}
            dataField="id"
            isKey={true}
            dataSort={true}
            sortFunc={championNameSorter}
          >
            Champion
          </TableHeaderColumn>
          <TableHeaderColumn dataField="totalSessionsPlayed" dataSort={true}>
            Games
          </TableHeaderColumn>
          <TableHeaderColumn
            dataAlign="center"
            dataField="totalSessionsWon"
            dataFormat={winLossFormatter}
          >
            W-L
          </TableHeaderColumn>
          <TableHeaderColumn
...
const winLossFormatter = (cell, row) => (
  <div>
    <div>
      {row.totalSessionsWon}
      W-
      {row.totalSessionsLost}
      L,
      {' '}
      {winPercentageBadge(row.totalSessionsWon, row.totalSessionsPlayed)}
    </div>
  </div>
);
...
const championIconFormatter = (cell, row) => (
  <div className="clearfix rankedStatsCellName">
    <i
      className={`${champ_icons['champion-' + cell]} ${championIcon.medium}`}
    />
  </div>
);
