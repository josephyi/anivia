...
const renderRecentGames = rows => rows.map(row => renderRecentGame(row));
...
const renderRecentGame = row => (
  <tr key={row.gameId}>
    ...
    <td>
      {playerRoleLabel(row['stats']['playerRole'])}
      {' '}
      {playerPositionLabel(row['stats']['playerPosition'])}
    </td>
    <td>
      {row['stats']['championsKilled'] || '0'}
      /
      {row['stats']['numDeaths'] || '0'}
      /
      {row['stats']['assists'] || '0'}
    </td>
    <td>{timeLabel(row['stats']['timePlayed'])} </td>
    <td>
      <Label bsStyle={row['stats']['win'] === true ? 'success' : 'danger'}>
        <TimeAgo date={row['createDate']} />
      </Label>
    </td>
  </tr>
);
