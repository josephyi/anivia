...
const ProfileRankedStats = (
  {summoner, rankedLeagues, aggregateRankedStats},
) => (
  <ListGroup>
...
const renderRankedLeagues = rows => rows.map(row => renderRankedLeague(row));
...
const renderEntries = (rows, tier) => rows.map(row => renderEntry(row, tier));
