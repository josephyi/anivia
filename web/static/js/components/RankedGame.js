...
const renderBans = bans => bans.map(ban => renderBan(ban.championId));

const renderBan = championId => (
  <ListGroupItem key={championId}>
    <i
      className={
        `${champ_icons['champion-' + championId]} ${championIcon.medium}`
      }
    />
  </ListGroupItem>
);
      ...
      <Col md={4} mdPull={participant.teamId == 100 ? 0 : 8}>
        <div className={participant.teamId == 100 ? 'text-right' : 'text-left'}>
          {summoners[canonicalize(participant.summonerName)]
            ? leagueWinLoss(
                rankedLeagues,
                summoners[canonicalize(participant.summonerName)]['id'],
              )
            : ''}
        </div>
      </Col>
