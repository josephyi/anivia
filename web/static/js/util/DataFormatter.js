  ...
  if (summonerLevel === 30 && league) {
    return (
      <span>
        ...
        ({league[0]['entries'][0]['leaguePoints']} LP)
      </span>
    );
  }

  if (summonerLevel < 30) return <span>Level {summonerLevel}</span>;
  else return <span>Unranked</span>;
};
...
export const leagueWinLoss = (rankedLeagues, summonerId) => {
  ...
  if (league) {
    ...
    return (
      <span>
        {wins}W-{losses}L<p>{winPercentageBadge(wins, wins + losses)}</p>
      </span>
    );
  } else
    return null;
...
export function winPercentageBadge(wins, games) {
  ...
  if (percentage >= 70) labelStyle = 'warning';
  else if (percentage >= 60) labelStyle = 'info';
  else if (percentage >= 50) labelStyle = 'primary';
