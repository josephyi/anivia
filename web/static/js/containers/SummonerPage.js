      ...
      return (
        <Grid>
          ...
          <Row>
            <Col xs={12} md={3}>
              <Profile
                aggregateRankedStats={aggregateRankedStats}
                summoner={summoner}
                rankedLeagues={rankedLeagues}
              />
            </Col>
...
function mapStateToProps(state, ownProps) {
  ...
  if (
    data === undefined ||
    data['summoners'] === undefined ||
    data['summoners'][summonerName] === undefined
  ) {
    return {
      region,
      summoner: {},
  ...
  const currentGameId = summoner && currentGamesMap
    ? currentGamesMap[summoner.id]
    : null;
  ...
  return {
    ...
    summoner,
    summoners,
    rankedStats: summoner && rankedStatsData
      ? rankedStatsData[summoner.id] || []
      : [],
    recentGames: summoner && recentGamesData
      ? recentGamesData[summoner.id] || []
      : [],
    aggregateRankedStats: summoner && aggregateRankedStatsData
      ? aggregateRankedStatsData[summoner.id]
      : {},
    rankedLeagues,
  };
}
